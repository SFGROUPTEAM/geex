...
public class LogUtil {
	 ...
	 * @param req
	 */
	public static void printRequestLog(String req) {
		writeMessage(LOG_STRING_REQ_MSG_BEGIN);
		writeMessage(req);
		writeMessage(LOG_STRING_REQ_MSG_END);
	}
	 ...
	 * @param
	 */
	public static void printResponseLog(Map<String, String> resParam) {
		writeMessage(LOG_STRING_RSP_MSG_BEGIN);
		Iterator<Entry<String, String>> it = resParam.entrySet().iterator();
		while (it.hasNext()) {
			Entry<String, String> en = it.next();
			writeMessage("[" + en.getKey() + "] = [" + en.getValue() + "]");
		}
